...
const ProfileInfo = () => {
    ...
    const logout = async () => {
        try {
        const res= await apiClinet.post(LOGOUT_ROUTE,{},{withCredentials:true})
        if(res.status===200){
            navigate('/auth')
            setUserInfo(null)
        }
  ...
  return (
    <div className=' absolute bottom-0 h-16 flex items-center justify-between w-full px-10 bg-[#2a2b33]  '>
        ...
        <Avatar className="h-12 w-12 rounded-full overflow-hidden">
            ...
            <TooltipProvider>
  <Tooltip>
    <TooltipTrigger><FiEdit2 className='text-purple-500 font-medium text-xl'
    ...
    <TooltipContent className='bg-[#1c1b1e] border-none  text-white'>
      <p>Edit profile</p>
            ...
            <TooltipProvider>
  <Tooltip>
    <TooltipTrigger><IoPowerSharp className='text-red-500  font-medium text-xl'
    onClick={logout}
    /></TooltipTrigger>
    <TooltipContent className='bg-[#1c1b1e] border-none  text-white'>
      <p>logout</p>
